...
const registerUser = asyncHandler(async (req,res) =>{
       ...
       //chack for files(avatar,cover image)
      ...
      let avatarLocalPath;
      if(req.files && Array.isArray(req.files.avatar) && req.files.coverImage.length >0){
    ...
    if(!createdUser){
        throw new ApiError(500,"Somwthing went wrong while regestering the user")
    }
...
const loginUser = asyncHandler(async (req,res) =>{
  ...
  //username or email
  //find the user
  //check the password and validate
...
const user = await  User.findById(req.user?._id)
   ...
   if(!isPasswordCorrect){
    throw new ApiError4(400,"Invalid old Password")
   }
